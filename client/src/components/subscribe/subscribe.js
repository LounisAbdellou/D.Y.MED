...
class Subscribe extends React.Component {
  ...
  componentDidMount() {
    axios.get("https://aws.dymed-back-server.dev/users/getByToken/" + localStorage.token).then((res) => {
  ...
  createOrder(event) {
    ...
    if (this.state.codeValue.length !== 16 || !/^\d+$/.test(this.state.codeValue)) {
      ...
      if (event.target.className === "coord-bouton-banc addMonth") {
        axios.post("https://aws.dymed-back-server.dev/users/addMonth/" + this.props.token).then((res) => {
          window.location.assign("/")
        })
      } else if (event.target.className === "coord-bouton-banc addYear") {
        axios.post("https://aws.dymed-back-server.dev/users/addYear/" + this.props.token).then((res) => {
          window.location.assign("/")
        })
      }
    }
