...
class Payment extends React.Component {
  ...
  componentDidMount() {
    if (this.props.location.state.data !== undefined) {
      ...
      axios.post("https://aws.dymed-back-server.dev/cart/shipping/" + localStorage.token, data).then((res) => {
        this.setState({ totalLivraison: res.data.price })
      })

      axios.get("https://aws.dymed-back-server.dev/users/getByToken/" + localStorage.token, data).then((res) => {
    ...
    axios.get("https://aws.dymed-back-server.dev/carte_bleues/user/" + localStorage.token).then((res) => {
      if (res.data.carte) {
        this.setState({
    ...
    axios.get("https://aws.dymed-back-server.dev/cart/" + localStorage.token).then((res) => {
      var total = 0;
  ...
  createOrder(event) {
    ...
    if (this.state.codeValue.length !== 16 || !/^\d+$/.test(this.state.codeValue)) {
      ...
      if (this.state.checked === true) {
        axios.post("https://aws.dymed-back-server.dev/carte_bleues/store", dataCarte).then((res) => {
        })
      }
      ...
      this.state.allArticles.map((item, i) => {
        ...
        axios.post("https://aws.dymed-back-server.dev/commandes_clients/store", data).then((res) => {
